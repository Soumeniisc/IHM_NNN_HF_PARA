...
VecDoub_O &x, Doub &f, const Doub stpmax, Bool &check, T &func) {
	...
	alam=1.0;
	for (;;) {
              // cout<<"         something i s happending -------------------";
		for (i=0;i<n;i++) x[i]=xold[i]+alam*p[i];
		f=func(x);
		if (alam < alamin) {
		cout<<"alam,alamin----------------------"<<alam<<"    "<<alamin<<endl;
			for (i=0;i<n;i++) x[i]=xold[i];
			check=true;
			return;
		} else if (f <= fold+ALF*alam*slope) return;
		else {
		       cout<< "x0,x1,xold0,xold1:"<<x[0]<<"    "<< x[1] <<"    "<< xold[0] <<"    "<< xold[1]<<endl;
                       cout<<"alam,alamin2----------------"<<alam<<"    "<<alamin<<" f "<<f<<"  fold "<<fold<<"   "<<(fold+ALF*alam*slope)<<endl;
			if (alam == 1.0)
...
void broydn(VecDoub_IO &x, Bool &check, T &vecfunc) {
	...
	const Doub TOLF=1.0e-6, TOLX=EPS, STPMX=100.0, TOLMIN=1.0e-6;
	Bool restrt,skip;
	...
	for (its=1;its<=MAXITS;its++) {
		...
		if (slope >= 0.0) {
			...
			continue;
                ...
                cout<<"iteration3,check:------------------------------------------"<<its<<"   "<<check<<endl;
		lnsrch(xold,fold,g,p,x,f,stpmax,check,fmin);
		test=0.0;
		for (i=0;i<n;i++)
			if (abs(fvec[i]) > test) test=abs(fvec[i]);
		if (test < TOLF) {
                cout<<"test< TOLLF:------------------------------------------"<<its<<endl;
			check=false;
			delete qr;
			return;
		}
                cout<<"iteration4:------------------------------------------"<<its<<endl;
		if (check) {
 		cout<<"check,restrt:------------------------------------------"<<check<<"   "<<restrt<<"   "<<its<<endl;
			if (restrt) {
                        cout<<"check,restrt2:------------------------------------------"<<check<<"   "<<restrt<<"   "<<its<<endl;
				delete qr;
				return;
			} else {
				test=0.0;
				...
				if (test < TOLMIN) {
					...
					return;
                        ...
                        cout<<"check,restrt3:------------------------------------------"<<check<<"   "<<restrt<<"   "<<its<<endl;
			restrt=false;
			test=0.0;
			for (i=0;i<n;i++) {
				temp=(abs(x[i]-xold[i]))/MAX(abs(x[i]),1.0);
